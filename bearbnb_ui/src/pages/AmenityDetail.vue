					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateAmenity()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div id="app">
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Amenitys = true">Add</base-button>
								              <modal :show.sync="modal2Amenitys">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Amenity Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="AmenityId" type="text" placeholder="Enter AmenityId" v-model="amenityDetails.amenityId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Name" type="text" placeholder="Enter Name" v-model="amenityDetails.name"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Description</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Description" type="text" placeholder="Enter Description" v-model="amenityDetails.description"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateAmenity() {
      ...
      const jsonData = JSON.stringify(this.amenityDetails);
      const res = await AmenityService.update(jsonData);
...
//        this.getAllAmenitys();
    ...
    async getAmenityDetails() {
      try {
			let response = await AmenityService.get(this.amenityId);
			this.amenityDetails = response.data;
